CREATE TRIGGER insert_user_data AFTER INSERT ON auth.users FOR EACH ROW EXECUTE FUNCTION insert_user_data();
...
create policy "Give anon users access to JPG images in folder public"
on "storage"."objects"
as permissive
for select
to public
using (((bucket_id = 'images'::text) AND (storage.extension(name) = 'jpg'::text)));


create policy "Give anon users access to JPG images in folder public_1"
on "storage"."objects"
as permissive
for insert
to public
with check (((bucket_id = 'images'::text) AND (storage.extension(name) = 'jpg'::text)));


create policy "Give anon users access to JPG images in folder public_2"
on "storage"."objects"
as permissive
for update
to public
